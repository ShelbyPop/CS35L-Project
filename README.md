...
> this string was only in place during development and is not present in the current version of 
> the code. Additionally, the password in the connection string in the commit history has been
> revoked, so it is no longer a valid connection string for our cluster.
